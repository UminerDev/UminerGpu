...
- The aligned panel reports both this-round and process-total A/S/R for every GPU and the rig.
- Interactive terminals use semantic colors without bold; `--no-color`, `NO_COLOR`, `TERM=dumb`
...
Version 0.1.18 is an active-fee testing release: BTX is 2% and CSD is 3%. The policy is compiled
...
- v0.1.18 is published under an explicit testing-stage exception. Long-duration production
...
- 對齊面板同時顯示每張 GPU 與整機的本輪 A/S/R 及進程累計 A/S/R。
...
v0.1.18 是啟用 Fee 的測試版：BTX 2%、CSD 3%。費率按幣種固化，CLI 無法下調。
...
- v0.1.18 根據測試階段特例直接發佈；生產級長時穩定驗收仍在進行，本版不宣稱已完成。
